...
def test_Predicate():
    ...
    assert not p == a(y, x)
    assert not p == a(x, y, z)
    assert p & q == And(p, b(z))
    assert Or(p) == p
...
def test_ForAll():
    ...
    assert ForAll((x, y, z), a(x) >> b(y, z)).expr == Implies(a(x), b(y, z))
    assert ForAll((x, y), ForAll(z, a(x, y) ^ b(y, z))).vars == (x, y, z)
    assert ForAll((x, y), ForAll(z, a(x, y) ^ b(y, z))).expr == Xor(a(x, y), b(y, z))
...
def test_Exists():
    ...
    assert Exists((x, y, z), a(x) >> b(y, z)).expr == Implies(a(x), b(y, z))
    assert Exists((x, y), Exists(z, a(x, y) ^ b(y, z))).vars == (x, y, z)
    assert Exists((x, y), Exists(z, a(x, y) ^ b(y, z))).expr == Xor(a(x, y), b(y, z))
