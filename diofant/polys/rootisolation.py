...
def dup_refine_real_root(f, s, t, K, eps=None, steps=None, disjoint=None):
    ...
    if s < 0:
        if t <= 0:
            f, s, t, negative = dmp_compose(f, [-K.one, K.zero], 0, K), -t, -s, True
        else:
...
def dup_inner_isolate_negative_roots(f, K, inf=None, sup=None, eps=None, mobius=False):
    ...
    roots = dup_inner_isolate_real_roots(dmp_compose(f, [-K.one, K.zero], 0, K), K, eps=eps)
...
class RealInterval:
    ...
    def __init__(self, data, f, dom):
        ...
        if len(data) == 2:
            ...
            if s < 0:
                if t <= 0:
                    f, s, t, self.neg = dmp_compose(f, [-dom.one, dom.zero], 0, dom), -t, -s, True
                else:
                    raise ValueError(f"can't refine a real root in ({s}, {t})")
